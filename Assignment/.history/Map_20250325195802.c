...
void findDirectPath(TransportGraph *graph, char *from, char *to, int *departTime) {
    ...
    for (int i = 0; i < graph->numWalkingLinks; i++) {
        if (strcmp(graph->walkingLinks[i].from, from) == 0 && strcmp(graph->walkingLinks[i].to, to) == 0) {
    ...
    for (int i = 0; i < graph->numFerrySchedule; i++) {
        if (strcmp(graph->ferrySchedule[i].from, from) == 0 && strcmp(graph->ferrySchedule[i].to, to) == 0 && graph->ferrySchedule[i].departTime >= departTime) {
